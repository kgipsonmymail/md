...
async function handleProcessFile(file: File) {
  try {
    const result = await windStore.processPdf(file, {
...
function closeDialog() {
  isShowImportPdfDialog.value = false
  isDragover.value = false
  resetFileDialog()
...
// 导出给外部使用
defineExpose({
  open: () => { isShowImportPdfDialog.value = true },
  close: closeDialog,
})
</script>

<template>
  <Dialog :open="isShowImportPdfDialog" @update:open="onOpenChange">
    <DialogContent class="sm:max-w-md">
      <DialogHeader>
        <DialogTitle>导入 PDF 文档</DialogTitle>
        <DialogDescription>
          将 .pdf 格式的文档转换为 Markdown，支持自动上传图片到图床
        </DialogDescription>
      </DialogHeader>
