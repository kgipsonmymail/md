...
function replaceToEditor() {
  if (!result.value || !editorStore.editor)
    return

  const editorView = editorStore.editor
  editorView.dispatch({
    changes: { from: 0, to: editorView.state.doc.length, insert: result.value.markdown },
  })

  editorView.focus()
  showConfirmDialog.value = false
  alert('AI排版内容已经更新在编辑器')
}
</script>
    ...
    <div class="processor-main">
      ...
      <div class="options-section">
        ...
        <div class="option-group">
          <label class="option-label">AI 提供商</label>
          <div class="provider-selector">
            ...
            <button
              :class="{ active: aiProvider === 'dify' }"
              @click="aiProvider = 'dify'"
            >
              Dify
            </button>
          </div>
      ...
      <div v-if="result" class="result-section">
        <div class="result-header">
          ...
          <div class="result-actions">
            ...
            <button class="btn-replace" @click="confirmReplace">
              替换到编辑器
            </button>
          </div>
        </div>
      ...
      <div v-if="showConfirmDialog" class="confirm-dialog-overlay" @click.self="showConfirmDialog = false">
        <div class="confirm-dialog">
          <h3>确认替换</h3>
          <p>确定要将处理结果替换到编辑器吗？这将覆盖编辑器中的现有内容。</p>
          <div class="dialog-actions">
            <button class="btn-cancel" @click="showConfirmDialog = false">
              取消
            </button>
            <button class="btn-confirm" @click="replaceToEditor">
              确认替换
            </button>
          </div>
        </div>
...
.provider-selector button.active {
  background: #4caf50;
  color: white;
  border-color: #4caf50;
}
