  ...
  <div v-if="dialogVisible" class="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center">
    <div class="bg-card text-card-foreground shadow-lg border rounded-xl w-[90vw] max-w-lg overflow-hidden flex flex-col max-h-[90vh] animate-in slide-in-from-bottom-4 relative">
      <div class="px-6 py-4 border-b flex items-center justify-between">
        ...
        <Button variant="ghost" size="sm" class="h-8 py-0 px-2 text-muted-foreground" @click="dialogVisible = false">
          ✕
        </Button>
      </div>
      <div class="px-6 py-4 overflow-y-auto flex-1 space-y-6">
        ...
        <!-- 图床配置 -->
        <div class="space-y-4">
          <h3 class="text-sm font-medium text-primary border-l-2 border-primary pl-2">
            GitHub 图床设置
          </h3>
          <div class="space-y-2">
            <Label>Personal Access Token</Label>
            <Input v-model="localConfig.github.token" type="password" placeholder="ghp_xxx 这将用于自动上传拖入的图片" />
          </div>
        ...
        <div class="space-y-4 border-t pt-4">
          ...
          <div class="space-y-2">
            <Label>API Key</Label>
            <Input v-model="localConfig.ai.apiKey" type="password" placeholder="sk-xxx" />
          </div>
          <div class="grid grid-cols-2 gap-4">
            <div class="space-y-2">
              <Label>调用模型</Label>
